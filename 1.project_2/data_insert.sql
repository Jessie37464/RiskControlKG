...
-- #导入实体和属性
    CALL apoc.periodic.iterate('CALL apoc.load.csv(\'./data/person.csv\') yield map as row return row',
                                'CREATE (person:PERSON) SET person = row', 
                                {batchSize:10000, iterateList:true, parallel:true})

    CALL apoc.periodic.iterate('CALL apoc.load.csv(\'./data/phone.csv\') yield map as row return row',
                                'CREATE (phone:PHONE) SET phone = row', 
                                {batchSize:10000, iterateList:true, parallel:true})

    CALL apoc.periodic.iterate('CALL apoc.load.csv(\'./data/apply_train.csv\') yield map as row return row',
                                'CREATE (apply:APPLY) SET apply = row', 
                                {batchSize:10000, iterateList:true, parallel:true})

    CALL apoc.periodic.iterate('CALL apoc.load.csv(\'./data/phone2phone.csv\') yield map as row return row',
                            'CREATE (phoneinfo:PHONEINFO) SET phoneinfo = row', 
                            {batchSize:10000, iterateList:true, parallel:true})
